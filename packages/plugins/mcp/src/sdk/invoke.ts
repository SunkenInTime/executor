    ...
    if (Exit.isSuccess(exit)) {
      ...
      return {
        ...
        ...(response.action === "accept" && response.content
          ...
          : {}),
        // The answer's terms ride back the way the request's came: in
        // `_meta`. Exactly the chosen scope and nothing else, so the wire
        // carries no more than the contract names; a decline carries none.
